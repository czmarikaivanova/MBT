...
\While{$S'\neq V$} {
	...
	\textbf{return} $\tau((V,F),S)$ \\
\caption{Construction of near-optimal solutions through sequences of matchings}
\label{alg:match}
\end{algorithm}

The algorithm generates a broadcast forest $(V,F)$ with source set $S$ consisting of trees, each with a single source (root). The broadcast time of such a tree can be determined recursively in polynomial time by the following Alg.~\ref{alg:treembt}.

\begin{algorithm}
\SetKwFunction{printlcs}{\textsc{Tree}-MBT}
	\Indm\nonl\printlcs{$T,s$}\\
\Indp
  $L\leftarrow \text{empty list}$\\
	\For {$v:\{s,v\}\in F, s\in S$} {
		$X\leftarrow\text{subtree of }(V,F) \text{ rooted at } v$\\
		$\tau\leftarrow$\printlcs{$X,v$}\\
		$\text{insert } \tau \text{ in } L \text{ such that } L \text{ remains a non-decrasing list of integers}$

		}

		\If{$L \text{ is empty}$}{\KwRet{0}}
		\KwRet{$\max\{k+L[k]:k=1,\dots,|L|\}$}
\caption{Polynomial procedure for finding broadcast time on trees}
\label{alg:treembt}

\end{algorithm}

The broadcast time $\tau((V,F),S)$ of the forest is then the maximum of broadcast times of these trees in $((V,F),S)$.

In many instances, there are multiple options for selecting an optimal solution $x$ (line 4).
Two different sequences of solutions generated during the course of Alg.~\ref{alg:match} may result in different broadcast times, as can be illustrated on the example of a hypercube~\ref{sect:hc}.
...
(TODO: This is no longer true for the improved Alg. 2. But let's wait for the experiments to see if there are other instances with such property.)
