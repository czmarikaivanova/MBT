...
It also sets correct values to the $z$-variables that appear in the objective function.
...
The (redundant) valid inequalities \eqref{mod:basic:timing} reflect that $k\leq\tau(G,S)$ only if $k-1\leq\tau(G,S)$.
...
If the optimal objective function value is $n-\sigma$, the corresponding value of $x$ induces a broadcast forest with broadcast time $t$.
...
\label{mod:basic:dec:tcrel} \sum\limits_{v\in N(u)}x_{uv}^k  \leq 1 &&  u\in S,k=1,\dots,t,\\
...
To pursue the constraint that sources inform at most one neighbor node in each period, \eqref{mod:basic:tcrel} is replaced by \eqref{mod:basic:dec:tcrel}.
