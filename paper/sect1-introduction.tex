...
This communication protocol appears in various practical applications, such as communication among computer processors or telephone networks.
...
Lower and upper bounding methods are derived in Sections \ref{sec:lb} and \ref{sec:ub}, respectively.
...
Deciding whether an instance of MBT has a solution with broadcast time at most $t$ has been shown to be NP-complete \cite{garey79,slater81}. 
...
Examples of additional efficient heuristics can be found e.g. in \cite{desousa18,harutyunyan06,harutyunyan14,wang10}.

Approximation algorithms for MBT are studied in \cite{kortsarz95}. 
The authors argue that methods presented in \cite{scheuermann84} provide no guarantee on the performance, and show that wheel-graphs are examples of unfavorable instances.
...
(Throughout the current text, the symbol $\log$ refers to the logarithm with base 2.)
...
McGarvey et al.\ \cite{mcgarvey16} study integer linear programming (ILP) models for $c$-broadcast graphs, which is a generalization where signal transmission to at most $c$ neighbours is allowed in a single time step.
