...
The parent function $\pi$ maps each node to the node from which it receives the signal.
...
% and $N_G(U)=\bigcup_{v\in U}N_G(v)$.
...
% Finally, we let $\deg^+_{\overrightarrow{G}}(v)=\left|N_{\overrightarrow{G}}^+(v)\right|$ and $\deg^-_{\overrightarrow{G}}(v)=\left|N_{\overrightarrow{G}}^-(v)\right|$ denote, respectively, the out-degree and the in-degree of $\overrightarrow{G}$,
